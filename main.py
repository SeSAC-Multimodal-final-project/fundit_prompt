...
# 🔐 API 키 로딩
load_dotenv(dotenv_path="/Users/minjoo/Desktop/SeSac/final/fundit_promport/.env")
openai.api_key = os.getenv("OPEN_API_KEY")

# 📄 정책 데이터 불러오기
with open("data/20250304.json", "r", encoding="utf-8") as f:
    policies = json.load(f)

# 🧠 LLM으로 정책 요약
def summarize_policy(policy):
    with open("prompts/policy_prompt.txt", "r", encoding="utf-8") as f:
...
def personalized_summary(user_info, summaries):
    with open("prompts/top_recommendation_prompt.txt", "r", encoding="utf-8") as f:
        template = f.read()
...
# 💾 사용자 맞춤 요약 저장
with open("data/user_summary.json", "w", encoding="utf-8") as f:
